<template>
  ...
  >
    <hack-content
    class="flex flex-col gap-10 justify-center flex-wrap"
      >
      <h2 class="text-center text-3xl">Отзывы</h2>
      <ul class="rounded-lg grid md:grid-cols-2 gap-5 grid-rows-auto ">
        <li
          class="items-center relative rounded-lg flex bg-accent"
        ...
        >
          ...
          <img
            class="rounded-s-lg h-full object-cover w-1/3"
            :src="config.public.apiUrl + '/static/images/' + review.author_img"
            :alt="review.author"
          />
...
import HackSection from "~/components/UI/HackSection.vue"
const config = useRuntimeConfig()
const { data } = await $fetch(`${config.public.apiUrl}/review/get`)
console.log(data)
</script>
