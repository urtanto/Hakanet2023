<template>
  <section v-if="data.length" class="flex flex-col gap-10 justify-center flex-wrap mb-10">
    <h2 class="text-center text-3xl">Отзывы</h2>
    <div class="rounded-lg grid grid-cols-2 gap-5 grid-rows-auto">
      <div class="items-center relative rounded-lg flex bg-accent" v-for="review in data" :key="review.id">
        <span
          class="absolute inline-block right-5 rotate-180 w-6 h-6 top-6 z-[2] opacity-50 text-accent-400 text-2xl font-bold">
        ...
        <img class="rounded-s-lg h-full" :src="review.author_img" :alt="review.author" />
        <div class="p-2 flex flex-col gap-2">
          <h3 class="text-xl font-bold tracking-tight text-default">
            <a href="#">{{ review.author }}</a>
          </h3>
          <p class="font-light text-default">
            {{ review.msg }}
          </p>
        </div>
      </div>
...
interface Review extends Object {
  id: number
  author: string
  msg: string
  author_img: string
}
const { data } = await $fetch("http://127.0.0.1:8000/review/get/")
</script>
